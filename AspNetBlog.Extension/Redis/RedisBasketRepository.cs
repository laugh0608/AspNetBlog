...
namespace AspNetBlog.Extension.Redis;

[Description("普通缓存考虑直接使用 ICaching，如果要使用 Redis 队列等还是使用此类")]
    public class RedisBasketRepository : IRedisBasketRepository
    {
        ...
        public async Task<TEntity> Get<TEntity>(string key)
        {
            ...
            if (value.HasValue)
            {
                // 需要用的反序列化，将 Redis 存储的 Byte[]，进行反序列化
